...
RangeQuery::RangeQuery(
    ...
    const void* start,
    const void* end)
    : order_{axis->label_order()}
    , label_data_size_{datatype_size(axis->label_dimension()->type())}
    , index_data_size_{datatype_size(axis->index_attribute()->type())}
    , input_label_range_{start, end, label_data_size_}
    , computed_label_range_{input_label_range_}
    , computed_index_range_{axis->index_dimension()->domain()}
    , lower_bound_query_{storage_manager, axis->labelled_array()}
    , upper_bound_query_{storage_manager, axis->labelled_array()} {
  if (order_ != LabelOrder::FORWARD)
    throw std::invalid_argument(
        "Support for reading ranges is only implemented for forward-ordered "
        "axes.");
  const auto label_dim = axis->label_dimension();
  const auto label_domain = label_dim->domain();
  const auto label_name = label_dim->name();
  const auto index_name = axis->index_attribute()->name();
